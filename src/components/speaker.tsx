...
const PlayButton = memo(
  ({
    ...
    customTrack?: { url: string; title: string }
  }) => {
    const player = useAudioPlayer()
    const item = customTrack
      ? player.activeItem
        ? {
            id: "custom",
            src: customTrack.url,
            data: { name: customTrack.title },
          }
        : undefined
      : player.activeItem
        ? {
...
const SpeakerContextBridge = ({
  ...
  return useMemo(
    ...
    [className, customTrack, showOrbs]
  )
}

export interface SpeakerProps {
  className?: string
  /** When provided, uses this single track instead of exampleTracks */
  track?: { url: string; title: string }
  /** When false, hides the orbs. Default true. */
  showOrbs?: boolean
}

export function Speaker({ className, track, showOrbs = true }: SpeakerProps) {
  return (
    <AudioPlayerProvider>
...
function SpeakerControls({
  ...
  useEffect(() => {
    if (customTrack) {
      ...
      precomputeWaveform(customTrack.url)
    } else {
      const track = {
  ...
  }, [precomputeWaveform, customTrack])
  ...
  const tracks = customTrack
    ? [{ id: "custom", title: customTrack.title, artist: "ElevenLabs Music" }]
    : exampleTracks.map((t) => ({
        id: t.id,
        title: t.name,
        artist: "ElevenLabs Music",
      }))
  const currentTrack = tracks[customTrack ? 0 : currentTrackIndex]
